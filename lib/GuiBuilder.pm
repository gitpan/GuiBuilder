...
GuiBuilder - The great new GuiBuilder!
...
For example, if user need to create GUI as depicted below within dotted box, then he need to select/specify following in GUI windows invoked by generate_gui method.
  ...
  => 2 Buttons named Button_OK and Button_Cancel at Frame/Row/Column 2/1/1 and 2/1/2 respectively.
    ...
    Above example explains the concept of Frame, Row and Column used here and How the fields are supposed to be entered. User can have everything into single frame or can have as many frames as number of rows. Mostly group of related fields should be having same frame number from GUI geometry point of view so that they can be positioned/controlled as a group.
    ...
    About 'autogenerated_gui.pl': 
      ...
      => It is having one subroutine named 'get_gui_options' which returns values of all the Options selcted by user in the form of list. Using values provided by this subroutine user can invoke its command line version of the script/utility.
...
generate_gui()
  Invokes GUI window to select various GUI fields required to generate GUI wrapper for perl script/utility.
